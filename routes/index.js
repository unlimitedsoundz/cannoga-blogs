...
router.get('/', async (req, res) => {
  try {
    ...
    const fullPage = await ejs.renderFile(path.join(__dirname, '../views/layouts/main.ejs'), {
      title: category ? `Blog - ${category} | Cannoga Blog` : (search ? `Search: ${search} | Cannoga Blog` : 'Cannoga Blog'),
      metaDescription: category ? `Read articles about ${category} from Cannoga College student ambassadors.` : 'Stories and insights from Cannoga College student ambassadors sharing their experiences studying and living in Canada.',
      ogImage: null,
      body: blogContent
    });
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    if (error || !post) {
      ...
      const fullPage = await ejs.renderFile(path.join(__dirname, '../views/layouts/main.ejs'), {
        title: 'Post Not Found | Cannoga Blog',
        body: errorContent
      });
